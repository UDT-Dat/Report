...
export class UserService {
    ...
    async create(createUserDto: CreateUserDto, currentUser?: User): Promise<User> {
        ...
        const user = await this.userModel.create({
            ...
            status: currentUser?.role === UserRole.ADMIN ? UserStatus.ACTIVE : UserStatus.PENDING,
        });
        return getInfoData(["_id", "email", "name", "role", "status"], user.toObject(), "_id") as User;
    }
    ...
    async findOne(id: string): Promise<User> {
        const user = await this.userModel.findById(id).select("-password -__v").lean();
        if (!user) {
    ...
    async update(id: string, updateUserDto: UpdateUserDto): Promise<User> {
        ...
        const { modifiedCount } = await user.updateOne(updateUserDto, { new: true }).exec()
        if (!modifiedCount) {
