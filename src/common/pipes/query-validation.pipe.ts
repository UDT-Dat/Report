...
export class QueryValidationPipe implements PipeTransform {
    ...
    constructor(
        ...
        this.allowedOperatorsSet = new Set([...allowedOperators, "lte", "gte", "lt", "gt", "ne", "like"]);
        this.eqOnlyFieldsSet = new Set([...eqOnlyFields, 'page', 'size', 'limit']);
    }

    transform(value: any) {
        value = this.parseFilterString(value)
        ...
        for (const key of Object.keys(value)) {
            ...
            if (!isAllowedAttr || (isEqOnly && op !== 'eq') || (!isEqOnly && !this.allowedOperatorsSet.has(op))) {
