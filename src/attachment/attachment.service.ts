...
export class AttachmentService {
  ...
  async uploadAttachment(files: Express.Multer.File[], ownerId: string, ownerType: 'Post' | 'Library', uploadedBy: string): Promise<Attachment[]> {
  ...
  async findAttachmentById(id: string) {
    return await this.attachmentModel.findById(toObjectId(id)).populate('ownerId');
  }
  async findAndDeleteAttachmentById(id: string){
    return await this.attachmentModel.findByIdAndDelete(toObjectId(id))
  }
  async getByOwerId({
    ownerId, ownerType, filter, pagination
  }: {
    ownerId: string,
    ownerType: 'Post' | 'Library',
    filter: object,
    ...
    return this.attachmentModel.find({ ownerId: toObjectId(ownerId), ownerType, ...filter })
      .populate({
        path: 'uploadedBy',
