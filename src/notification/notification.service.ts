...
export class NotificationService {
  ...
  async createEventNotification(eventId: string, eventTitle: string, users: User[]): Promise<void> {
    const title = 'New Event';
    const message = `A new event "${eventTitle}" has been created. Check it out!`;
    
    for (const user of users) {
      await this.create(title, message, NotificationType.NEW_EVENT, user, eventId);
    }
  }

  async createPostNotification(postId: string, postTitle: string, users: User[]): Promise<void> {
    const title = 'New Post';
    const message = `A new post "${postTitle}" has been published. Check it out!`;
    
    for (const user of users) {
      await this.create(title, message, NotificationType.NEW_POST, user, postId);
    }
  }
  ...
  async createAccountApprovedNotification(user: User): Promise<void> {
    ...
    const message = 'Your account has been approved! You now have full access to the platform.';
