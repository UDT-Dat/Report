...
export class LibraryService {
  ...
  async findAllLibraries(user: User): Promise<Library[]> {
    try {
      const isAdmin = [UserRole.ADMIN, UserRole.MENTOR].includes(
  ...
  async updateLibrary(
    ...
    try {
      ...
      const isAdmin = [UserRole.MENTOR].includes(user.role as UserRole);
      const isCreator = library.createdBy.toString() === user.userId.toString();
      if (!isAdmin || !isCreator) {
        throw new ForbiddenException(
  ...
  async uploadAttachment({
    ...
    try {
      ...
      const isMentor = [UserRole.MENTOR].includes(user.role as UserRole);
