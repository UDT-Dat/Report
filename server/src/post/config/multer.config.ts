...
export const multerConfig: MulterOptions = {
  ...
  fileFilter: (req, file, cb) => {
    ...
    // ⚠️ Chỉ filter mimetype của bannerImage
    if (file.fieldname === 'bannerImage') {
      if (!file.mimetype.match(/^image\/(jpg|jpeg|png|gif)$/)) {
        return cb(
          new BadRequestException(
            'Only image files are allowed for bannerImage (jpg, jpeg, png, gif)',
          ),
          false,
        );
      }
    ...
    // Các field khác như attachments sẽ không bị kiểm tra mimetype
    cb(null, true);
  },
  limits: {
