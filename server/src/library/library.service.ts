...
export class LibraryService {
  ...
  async findAllLibraries(user: User, query: object): Promise<LibraryListResponseDto> {
    const { result: filter, errors, pagination } = convertParam(query);
    try {
      if (errors.length > 0) {
        throw new BadRequestException(errors.join('; '));
      }
      const isAdmin = [UserRole.ADMIN, UserRole.BOD].includes(
  ...
  async uploadAttachment({
    ...
    try {
      ...
      const isMentor = [UserRole.BOD].includes(user.role as UserRole);
  ...
  async getAttachmentsByLibrary(
    libraryId: string,
    query: object & { fileType?: 'document' | 'image' | 'video' | 'audio' | 'other' },
    user: User,
    ...
    try {
      ...
      const fileTypeQuery = this.getQueryByFileType(query?.fileType as 'document' | 'image' | 'video' | 'audio' | 'other');
  ...
  async deleteAttachment(id: string, user: User): Promise<Attachment | null> {
    try {
      ...
      const isMentor = user.role === UserRole.BOD;
  ...
  async grantLibraryAccess(
    ...
    try {
      ...
      if (existingPermission) {
        throw new BadRequestException('User already has access to this library');
      }
  ...
  private getQueryByFileType(type: 'document' | 'image' | 'video' | 'audio' | 'other') {
    switch (type) {
      case 'document':
        return {
          fileType: {
      ...
      case 'video':
        return {
          fileType: {
            $in: ["video/mp4", "video/mpeg", "video/mpg", "video/avi", "video/mov", "video/wmv", "video/flv", "video/webm"]
          }
        };
      case 'audio':
        return {
          fileType: {
            $in: ["audio/mpeg", "audio/mp3", "audio/mpga", "audio/m4a", "audio/wav", "audio/webm"]
          }
        };
      case 'other':
        return {
          fileType: {
            $nin: ["application/pdf", "image/jpeg", "image/png", "video/mp4", "audio/mpeg"]
          }
        };
      default:
        return {};
