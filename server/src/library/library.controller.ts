...
export class LibraryController {
  ...
  async findAllLibraries(@UserDecorator() user: User, @Query() query: any): Promise<LibraryListResponseDto> {
  ...
  async getAttachments(
    ...
    const validatedQuery = await this.attachmentQueryPipe.transform(
      query,
    );
  ...
  @UseGuards(RolesGuard)
  @Roles(UserRole.BOD)
  @ApiOperation({ summary: 'Get all users with their access status for a library' })
  @ApiResponse({
    status: 200,
  ...
  @ApiQuery({ name: 'status', required: false, enum: ['granted', 'not_granted'], description: 'Filter by access status' })
