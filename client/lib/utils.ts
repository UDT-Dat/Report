...
export async function updateContent(
	...
	for (const image of images) {
		...
		// skip if src is not a blob url and get remain string from "/uploads/images/". Example: http://localhost:3000/uploads/images/123.jpg -> /uploads/images/123.jpg
		if (src && !src.startsWith('blob:') && src.includes('/uploads/images/')) {
			const remain = src.split('/uploads/images/')[1];
			image.setAttribute('src', `/uploads/images/${remain}`);
			continue;
		}
		if (src && src.startsWith('blob:')) {
			...
			if (res.path) {
				image.setAttribute('src', res.path);
			} else {
...
export const updateSrcImage = (htmlString: string): string => {
	...
	for (const image of images) {
		...
		if (src && src.startsWith('/uploads/images/')) {
			image.setAttribute('src', `${process.env.NEXT_PUBLIC_API_URL}${src}`);
		}
	}
	return doc.body.innerHTML;
};
