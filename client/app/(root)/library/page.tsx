...
export default async function LibraryPage() {
  ...
  const { libraries, pagination }: { libraries: LibraryItem[], pagination: { total: number, page: number, limit: number } } = await getLibrary(accessToken, refreshToken)
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-5">Thư viện của tôi</h1>
      {libraries?.length > 0 ? <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
        {libraries.map((item) => (
          <Card key={item._id}>
            <CardHeader>
            ...
            <CardContent className='flex flex-col gap-2'>
              ...
              <div className='flex items-center gap-2'>
                <Avatar>
                  <AvatarImage src={`${process.env.NEXT_PUBLIC_API_URL}/${item.createdBy?.avatar}`} alt={item.createdBy?.name} className='object-cover' />
