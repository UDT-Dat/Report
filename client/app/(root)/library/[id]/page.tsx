...
export default function LibraryPage({ params, searchParams }: { params: Promise<{ id: string }>, searchParams: Promise<{ fileType: string }> }) {
  ...
  const getAttachments = async ({ fileType }: { fileType: string }) => {
    ...
    const response = await fetchApi(`/library/${id}/attachments${query}`, {
      method: 'GET',
    }, {
  ...
  const handleDelete = async (id: string) => {
    const response = await fetchApi(`/library/attachments/${id}`, {
      method: 'DELETE',
    }, {
      accessToken: tokens?.accessToken,
      refreshToken: tokens?.refreshToken,
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetchApi(`/library/${id}/attachments`, {
        method: 'POST',
        body: formData,
      }, {
        accessToken: tokens?.accessToken,
  ...
  function canDelete(item: Attachment) {
    return item.uploadedBy?._id === user?._id || user?.role === UserRole.ADMIN || library?.createdBy?._id === user?._id
  }
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="mb-8">
        ...
        <p className="text-gray-500 dark:text-white">Kho tài liệu, hình ảnh, video và tài nguyên của CLB IT VLU</p>
      </div>
      ...
      <div className="inline-flex w-auto gap-2 bg-gray-200 p-2 rounded-md mb-6">
        {fileTypes.map((item) => (
          <Button variant="outline" size="sm" key={item.label} className={
            cn(
              "flex-shrink-0",
              item.value === fileType ? "bg-primary text-primary-foreground hover:bg-primary/90" : "bg-background text-foreground hover:bg-primary/70 hover:text-white"
            )
          } onClick={() => router.push(`/library/${id}?${item.value ? `fileType=${item.value}` : ""}`)}>
            {item.label}
          </Button>
        ))}
      </div>
          ...
          <Card key={item._id} className="overflow-hidden">
            <div className="relative aspect-video w-full overflow-hidden bg-gray-100">
              ...
              {item.fileType.includes("image") ? (<>
                <Image
                  src={`${process.env.NEXT_PUBLIC_API_URL}/${item.url}` || "/placeholder.svg"}
              ...
              {
                canDelete(item) && (
                  <div className='ml-auto size-10 flex items-center justify-center rounded-full bg-red-500 text-white hover:bg-red-600 z-50 absolute top-2 right-2' onClick={() => handleDelete(item._id)}>
            ...
            <CardContent className="space-y-2 p-4 relative">
              ...
              <div className="mb-3 flex gap-1 items-center">
                <Avatar>
                  <AvatarImage src={`${process.env.NEXT_PUBLIC_API_URL}/${item.uploadedBy?.avatar}`} />
                  <AvatarFallback>{item.uploadedBy?.name.charAt(0) || "U"}</AvatarFallback>
                </Avatar>
                <div>
                  <p className="text-sm text-gray-500 font-medium">{item.uploadedBy?.name || "Người dùng không tồn tại"}</p>
              ...
              <div className="flex space-x-2">
                <Link href={`${process.env.NEXT_PUBLIC_API_URL}/${item.url}`} download={`${process.env.NEXT_PUBLIC_API_URL}/${item.url}`} className='w-full'>
      ...
      <Dialog open={openDialog} onOpenChange={setOpenDialog}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Chọn tài liệu để tải lên thư viện. Hỗ trợ các định dạng: PDF, Word, Excel, PowerPoint, hình ảnh, video, audio.
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="max-h-32 overflow-y-auto space-y-2">
                  {Array.from(selectedFiles).map((file, index) => (
                    <div key={index} className="flex items-center justify-between p-2 bg-muted rounded-md">
                      ...
                      <div className='size-10 flex items-center justify-center rounded-full bg-red-500 text-white hover:bg-red-600' onClick={() => removeFile(index)}>
