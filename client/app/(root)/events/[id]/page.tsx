  ...
  const handleUnregister = async (e: React.FormEvent) => {
    ...
    if (tokens?.accessToken) {
      ...
      if ("error" in response.result) {
        ...
        setEvent({
          ...event,
          participants: event?.participants?.filter((participant) => participant !== user?._id) || [],
        } as Event)
      }
    }
  }
      ...
      <div className="grid gap-8 md:grid-cols-3">
        <div className="md:col-span-2">
          <div className="relative mb-6 aspect-[16/9] w-full overflow-hidden rounded-lg">
            <Image
              src={`${process.env.NEXT_PUBLIC_API_URL}/${event.imageUrl}` || "/placeholder.svg"}
          ...
          <Card className="sticky top-6">
            <CardContent className="p-6">
              <div className="mb-6">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Còn {event.maxParticipants - (event.participants?.length ?? 0)} chỗ trống. Hãy đăng ký ngay!
                </p>
              </div>
            ...
            <CardFooter className="flex flex-col gap-4 items-start">
              <div className="mt-6">
                ...
                <div className="flex items-center">
                  <div className="mr-3 h-10 w-10 overflow-hidden rounded-full">
                    <Image
                      src={
                        `${process.env.NEXT_PUBLIC_API_URL || "/placeholder.svg"}/${event.createdBy?.avatar}` ||
                        "/vltc-logo.png"
                      }
              ...
              {new Date() < new Date(event.endDate) && (
                <>
                  {isParticipant ? (
                    <Button variant="outline" className="w-full" disabled={isRegistering} onClick={handleUnregister}>
                      {isRegistering ? "Đang hủy..." : "Hủy đăng ký"}
                    </Button>
                  ) : (
                    <Button type="submit" className="w-full" disabled={isRegistering} onClick={handleRegister}>
